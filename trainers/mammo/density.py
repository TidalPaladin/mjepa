...
from mjepa.data.tiff import PreprocessedTIFFDataset, Transform
...
class DensityDataset(PreprocessedTIFFDataset):
    ...
    def __init__(
        ...
        super().__init__(root, training, transform, keep_volume)

        # Open study metadata
        study_metadata_path = self.root / "studies.parquet"
        if not study_metadata_path.is_file():
            raise FileNotFoundError(f"Study metadata file {study_metadata_path} does not exist")
        self.study_metadata = pd.read_parquet(study_metadata_path)
        self.study_metadata.set_index("study_instance_uid", inplace=True)
        assert not self.study_metadata.index.duplicated().any(), "Found duplicate study instance UIDs"

        # Open image metadata
        image_metadata_path = self.root / "images.parquet"
        if not image_metadata_path.is_file():
            raise FileNotFoundError(f"Image metadata file {image_metadata_path} does not exist")
        self.image_metadata = pd.read_parquet(image_metadata_path)
        self.image_metadata.set_index("sop_instance_uid", inplace=True)
        self.image_metadata = self.image_metadata.drop_duplicates(keep="first")

        # Ensure we only use images that are present in the study metadata
        self.manifest = self.manifest[self.manifest["study_instance_uid"].isin(self.study_metadata.index)]
        #self.manifest = self.manifest[self.manifest["sop_instance_uid"].isin(self.image_metadata.index)]

        #self.image_metadata = self.image_metadata[self.image_metadata["is_mlo_like"]]
        #self.manifest = self.manifest[self.manifest["sop_instance_uid"].isin(self.image_metadata.index)]

        # FIXME
        if training:
            #IGNORE_DATA_SOURCES = ["OMI-DB", "Cairde", "Fuji", "Hamad"]
            pass
            #IGNORE_DATA_SOURCES = ["Fuji", "OMI-DB"]
            #self.study_metadata = self.study_metadata[self.study_metadata["data_source"].isin(IGNORE_DATA_SOURCES)]
            #self.manifest = self.manifest[self.manifest["study_instance_uid"].isin(self.study_metadata.index)]
        else:
            IGNORE_DATA_SOURCES = ["UTSW"]
            self.study_metadata = self.study_metadata[self.study_metadata["data_source"].isin(IGNORE_DATA_SOURCES)]
            self.manifest = self.manifest[self.manifest["study_instance_uid"].isin(self.study_metadata.index)]

    def _density_to_tensor(self, density: str) -> Tensor:
        match density.strip().lower():
            case "a":
                t = torch.tensor(0)
            case "b":
                t = torch.tensor(0.33)
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, Tensor]:
        ...
        label = self.study_metadata.loc[study_uid, "breast_density"]
        label = self._density_to_tensor(label)
        return image, label
...
def get_train_dataloader(
    ...
    dataset = DensityDataset(root=root, transform=transforms, training=True)
    if world_size > 1:
...
def get_val_dataloader(
    ...
    dataset = DensityDataset(root=root, transform=transforms)
    if world_size > 1:
        sampler = DistributedSampler(dataset, num_replicas=world_size, rank=local_rank, shuffle=True)
        return DataLoader(
            dataset, batch_size=batch_size, num_workers=num_workers, sampler=sampler, persistent_workers=True
...
def density_score_to_category(score: Tensor) -> Tensor:
    return torch.round(score * (NUM_CLASSES - 1)).long()

def density_category_to_weight(category: Tensor) -> Tensor:
    # Inverse of 10/40/40/10 distribution
    return torch.tensor([0.4, 0.1, 0.1, 0.4], device=category.device)[category]
...
def train(
    ...
    train_acc_binary = Running(tm.Accuracy(task="binary"), window=WINDOW).cuda()
    val_acc = tm.Accuracy(task="multiclass", num_classes=NUM_CLASSES).cuda()
    val_acc_binary = tm.Accuracy(task="binary").cuda()
    val_auroc = tm.AUROC(task="binary").cuda()
    logger = CSVLogger(
    ...
    for epoch in range(last_epoch + 1, trainer_config.num_epochs):
        ...
        for img, label in pbar:
            ...
            img = img.cuda()
            label = label.cuda()
            weight = density_category_to_weight(density_score_to_category(label))
            if microbatch == 0:
                image_saver(img)
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                loss = bce_mixup(
                    pred.float(), label.unsqueeze(1).float(), mixup_seed, augmentation_config.mixup_prob, augmentation_config.mixup_alpha
                ).mean()
            ...
            with torch.no_grad():
                ...
                pred_category = density_score_to_category(pred_score)
                target_category = density_score_to_category(label)
                mask = is_mixed(
                    B, augmentation_config.mixup_prob, augmentation_config.mixup_alpha, mixup_seed
                ).logical_not_()
                train_acc.update(pred_category[mask].flatten(), target_category[mask].flatten())
                train_acc_binary.update(pred_score[mask].flatten(), label[mask].round().flatten())
            ...
            desc = format_pbar_description(step, microbatch, epoch, loss=train_loss, acc=train_acc, acc_binary=train_acc_binary)
        ...
        if (epoch + 1) % trainer_config.check_val_every_n_epoch == 0:
            ...
            for img, label in tqdm(val_dataloader, desc=f"Validating: ", disable=not is_rank_zero(), leave=False):
                ...
                with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    pred_category = density_score_to_category(pred_score)
                    target_category = density_score_to_category(label)
                    val_acc.update(pred_category.flatten(), target_category.flatten())
                    val_auroc.update(pred_score.flatten(), label.round().flatten())
                    val_acc_binary.update(pred_score.flatten(), label.round().flatten())

            # Validation epoch end
            rank_zero_info(f"Epoch: {epoch}, Val Acc: {val_acc.compute():.4f}, Val Acc Binary: {val_acc_binary.compute():.4f}, Val AUROC: {val_auroc.compute():.4f}")
            logger.log(epoch, step, microbatch, acc=val_acc.compute(), acc_binary=val_acc_binary.compute(), auroc=val_auroc.compute())
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("dev_data", type=Path, help="Path to development data")
    parser.add_argument(
    ...
    if args.checkpoint:
        ...
        if args.fresh_head:
            for head in backbone.heads.values():
                head.pool.reset_parameters()
                head.proj.reset_parameters()
