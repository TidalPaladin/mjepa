...
from torchvision.tv_tensors import Image as TVImage, Video as TVVideo
from torchvision.transforms.v2 import RandomHorizontalFlip

from mjepa.data.tiff import load_and_wrap_pixels, PreprocessedTIFFDataset
...
@pytest.mark.parametrize("training,keep_volume,expected_type", [
...
def test_preprocessed_dataset_getitem(preprocessed_root, mock_tiff_data, training, keep_volume, expected_type):
    ...
    dataset = PreprocessedTIFFDataset(
        preprocessed_root, 
        training=training, 
    ...
    with patch("mjepa.data.tiff.load_tiff_f32", return_value=mock_tiff_data["single_frame" if expected_type == TVImage else "multi_frame"]):
        result = dataset[0]
